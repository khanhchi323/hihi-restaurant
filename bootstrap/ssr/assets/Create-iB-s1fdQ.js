  ...
  setup(__props) {
    ...
    const submit = () => {
      ...
      form.post(route("reservation.store"), {
        ...
        preserveState: false,
        // Không giữ lại trạng thái của form sau khi post
        forceFormData: true
        // Bắt buộc sử dụng FormData
      });
    };
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_sfc_main$1, null, {
        header: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<h2 class="font-semibold text-xl text-gray-800 leading-tight"${_scopeId}> Edit Reservation </h2>`);
          } else {
            return [
              createVNode("h2", { class: "font-semibold text-xl text-gray-800 leading-tight" }, " Edit Reservation ")
            ];
          }
        }),
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`<button type="submit" class="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600"${_scopeId}> Save </button></div></div></form></div></div></div></div>`);
          } else {
            return [
